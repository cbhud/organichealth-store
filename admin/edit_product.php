...
if (isset($_POST['ajax'])) {
    ...
    $stmt = $konekcija->prepare("UPDATE product SET name=?, price=?, stock=?, description=?, img_url=?, category_id=?, inRecommended=? WHERE product_id=?");
    $stmt->bind_param('sdissiii', $name, $price, $stock, $description, $image_url, $category_id, $inRecommended, $product_id);
    if ($stmt->execute()) {
        $stmt->close();
        echo "OK";
        exit;
            ...
            <div class="create-product-form">
                ...
                <form class="edit-product-form">
                    ...
                    <textarea class="description" name="description" id="description"
                        maxlength="800"><?php echo $product['description'] ?></textarea>

                    <label for="image_url">Slika (URL)</label>
                    <input type="text" class="image_url" name="image_url" id="image_url" maxlength="300"
                        placeholder="Npr: slike/proizvod.jpg" value="<?php echo $product['img_url'] ?>">

                    <label style="display: flex; align-items: center; gap: 8px; margin-bottom: 16px;">
                        <input type="checkbox" name="inRecommended" id="inRecommended" style="width: auto; margin: 0;"
                            <?php echo (isset($product['inRecommended']) && $product['inRecommended']) ? 'checked' : ''; ?>>
                        <span>Prikaži na početnoj <small>(Izdvajamo iz ponude)</small></span>
                    </label>

                    <button type="button" onclick="validateProductForm(<?php echo $product_id; ?>)">Sacuvaj
                        izmjene</button>
                </form>
            </div>
        </div>
